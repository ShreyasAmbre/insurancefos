...
@Component({
  selector: 'app-wyn-hospital',
  templateUrl: './wyn-hospital.component.html',
  styleUrls: ['./wyn-hospital.component.scss']
})
export class WynHospitalComponent  implements OnInit {
  @Input() actonSelect: string = '';
  @Input() actionSelected: string = '';


 
/*  selectedDocuments!: documentListOptions[];*/
  ...
  healthCustomerLocateOnChange(){
    this.healthCustomerLocateForm.valueChanges.subscribe(formValue => {
      ...
      this.ppncreateservice.healthCustomerLocateSubject.next({ ...formValue, isValid: this.healthCustomerLocateForm.status })
  ...
  verifyPincode() {
    ...
    this.commonService.getVerifyPincode(this.healthCustomerLocateForm.get('hospitalPinCode')?.value).subscribe((res: any) => {
      if (res.success) {
        ...
        this.healthCustomerLocateForm.patchValue({ zoneId: res.ZoneAccessArea })
        
        this.ppncreateservice.zoneData['zoneId'] = this.healthCustomerLocateForm.get('zoneId')?.value
        console.log("response ZONEID ===>", this.healthCustomerLocateForm.get('zoneId')?.value)
